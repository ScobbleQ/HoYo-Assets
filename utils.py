...
def update_starrail():
    ...
    for id_str in data:
        starrail_icon_art(current_dir, id_str)
        starrail_splash_art(current_dir, id_str)
        starrail_wish_art(current_dir, id_str)
...
def starrail_icon_art(current_dir, character):
    try:
        response = requests.get(link_starrail_icon(character))
        character_icon = Image.open(BytesIO(response.content))

        text_files_dir = os.path.join(current_dir, "starrail", "icon")
        os.makedirs(text_files_dir, exist_ok=True)

        output_path = os.path.join(text_files_dir, f'{character}.png')
        character_icon.save(output_path)
    except Exception as e:
        print(f"Failed to create icon art for {character}: {str(e)}")

def starrail_splash_art(current_dir, character):
    try:
        response = requests.get(link_starrail_art(character))
        character_art = Image.open(BytesIO(response.content))
...
def starrail_wish_art(current_dir, character):
    try:
        response = requests.get(link_starrail_art(character))
        character_image = Image.open(BytesIO(response.content))
